	...
	while(!obtWindow.shouldClose()) {
		...
		if (auto commandBuffer = obtRenderer.beginFrame()) {
			...
			for (int i = 0; i < 3; ++i) {
				...
				lightData[i].position.x = i;
				lightData[i].position.z = 3-i;
				lightData[i].color.x = 1.f/(i+1);
				if (i == 1) lightData[i].color.y = 0.f;
			}
...
void App::loadGameObjects() {
	...
	for (int i = 0; i < 3; ++i) {
		auto g = ObtGameObject::createGameObject();
		g.model = obtModel;
		g.transform.translation = glm::vec3{-1.f};
		g.transform.translation.x = i;
		g.transform.translation.z = 3-i;
		g.transform.scale = glm::vec3{.1f};
